...
import com.legend.imkit.videocall.util.NetworkConnectionChecker
import com.legend.imkit.util.RingtonePlayer
import com.legend.imkit.videocall.util.WebRtcSessionManager
import com.legend.imkit.util.applyRTCSettings
...
import com.legend.imkit.videocall.fragments.CAMERA_ENABLED
import com.legend.imkit.videocall.fragments.IS_CURRENT_CAMERA_FRONT
import com.legend.imkit.videocall.fragments.MIC_ENABLED
import com.legend.imkit.videocall.fragments.SPEAKER_ENABLED
...
//    private var TAG = CallService::class.java.simpleName
    ...
    private var callTimerListener = mutableListOf<CallTimerListener>()
    ...
    companion object {
        private var TAG = "qbVideo-CallService"
...
//        Log.i(TAG, "CallService onStartCommand")
    ...
    private fun initNotification(): Notification {
        ...
        val channelId: String = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            getString(com.legend.commonres.R.string.app_name)
        }
    ...
    private fun createNotificationChannel(channelId: String, channelName: String): String {
        ...
        channel.lightColor = getColor(com.legend.commonres.R.color.primary_color)
...
//        Log.i(TAG, "CallService -- pepareToProcessCalls() ---")
    ...
    private fun initAudioManager() {
        ...
        appRTCAudioManager.setOnWiredHeadsetStateListener { plugged, hasMicrophone ->
            Log.i(TAG, "头戴式耳机 " + if (plugged) "plugged" else "unplugged")
        }

        appRTCAudioManager.setBluetoothAudioDeviceStateListener { connected ->
            Log.i(TAG, "蓝牙耳机 " + if (connected) "connected" else "disconnected")
        }

        appRTCAudioManager.start { selectedAudioDevice, availableAudioDevices ->
            Log.i(TAG, "语音设备转换 to  $selectedAudioDevice")
            if (selectedAudioDevice == AppRTCAudioManager.AudioDevice.SPEAKER_PHONE) {
                if (MMKVUtils.getBoolean(SPEAKER_ENABLED, false)) {
                    LiveEventBus.get<Boolean>(EventKey.key_qb_enable_phone_speaker).post(true)
                } else {
                    switchAudio(false)
                }
            } else {
                LiveEventBus.get<Boolean>(EventKey.key_qb_enable_phone_speaker).post(false)
            }
        }
    ...
    fun switchAudio(enableSpeaker: Boolean) {
        Log.v(TAG, "onSwitchAudio(), SelectedAudioDevice() = " + appRTCAudioManager.selectedAudioDevice)
        if (enableSpeaker && appRTCAudioManager.selectedAudioDevice == AppRTCAudioManager.AudioDevice.SPEAKER_PHONE) return
        if (!enableSpeaker && appRTCAudioManager.defaultAudioDevice != AppRTCAudioManager.AudioDevice.SPEAKER_PHONE) return
    ...
    fun setCallTimerCallback(callback: CallTimerListener) {
        if (!callTimerListener.contains(callback)) {
            callTimerListener.add(callback)
        }

    }

    fun removeCallTimerCallback(timeCallBack: CallTimerListener) {
        callTimerListener.remove(timeCallBack)
    }
    ...
    private fun startCallTimer() {
        ...
        if (!callTimerTask.isRunning) {
            if (callTimer == null) callTimer = Timer()
            callTimer?.scheduleAtFixedRate(callTimerTask, 0, 1000)
        }
    }

    private fun stopCallTimer() {
        callTimerListener.clear()
    ...
    fun clearButtonsState() {
        ...
        MMKVUtils.remove(IS_CURRENT_CAMERA_FRONT)
    }

    fun clearCallState() {
        MMKVUtils.remove(EXTRA_IS_INCOMING_CALL)
    }
    ...
    private inner class SessionEventsListener : QBRTCClientSessionCallbacks {
        override fun onUserNotAnswer(session: QBRTCSession?, userId: Int?) {
            Log.i(TAG, "CallService --- onUsrNotAnswer 无用户应答 ---")
            MainRequest.reportCallState(TypeConst.call_state_no_answer, WebRtcSessionManager.callUid?:"")
            WebRtcSessionManager.removeMultiCallUser(userId)
            stopRingtone()
        }

        override fun onSessionStartClose(session: QBRTCSession?) {
            Log.i(TAG, "CallService --- onSessionStartClose 通话会话即将结束 --- ")
        }

        override fun onReceiveHangUpFromUser(session: QBRTCSession?, userID: Int?, userInfo: MutableMap<String, String>?) {
            Log.i(TAG, "CallService --- onReceiveHangUpFromUser 被 $userID 挂断 --- ")
            stopRingtone()
...
//            ToastUtils.show("User " + participantName + " " + getString(R.string.text_status_hang_up) + " conversation")
        ...
        override fun onChangeReconnectionState(session: QBRTCSession?, userId: Int?, state: QBRTCTypes.QBRTCReconnectionState?) {
            Log.i(TAG, "CallService --- onChangeReconnectionState 重新连接状态 --- state = $state")
        }

        override fun onCallAcceptByUser(session: QBRTCSession?, userId: Int?, userInfo: MutableMap<String, String>?) {
            Log.i(TAG, "CallService --- onCallAcceptByUser 被 $userId 接听 --- ")
            stopRingtone()
            if (session != WebRtcSessionManager.getCurrentSession()) {
                return
            }
        ...
        override fun onReceiveNewSession(session: QBRTCSession?) {
            Log.i(TAG, "CallService --- onReceiveNewSession 有新会话呼入 --- session: "  + session?.userInfo)
            WebRtcSessionManager.getCurrentSession()?.let {
                Log.d(TAG, "Stop new session. Device now is busy")
                if (session?.userInfo != null) {
                    MainRequest.reportCallState(TypeConst.call_state_refuse_busy, session.userInfo["callUid"]?:"")
                }
                val userInfo = HashMap<String, String>()
                userInfo["reason"] = TypeConst.type_call_reject_busy
                session?.rejectCall(userInfo)
            }
        }

        override fun onUserNoActions(p0: QBRTCSession?, userId: Int?) {
            Log.i(TAG, "CallService --- onUserNoActions 用户超时未响应 --- ")
            WebRtcSessionManager.removeMultiCallUser(userId)
            clearCallState()
            clearButtonsState()
        ...
        override fun onSessionClosed(session: QBRTCSession?) {
            Log.i(TAG, "CallService --- onSessionClosed 会话被关闭 --- ")
        ...
        override fun onCallRejectByUser(session: QBRTCSession?, userId: Int?, userInfo: MutableMap<String, String>?) {
            Log.i(TAG, "CallService --- onCallRejectByUser 被 $userId 拒绝接听 --- y")
            stopRingtone()
            if (session != WebRtcSessionManager.getCurrentSession()) {
                return
            }
    ...
    private inner class SessionStateListener : QBRTCSessionStateCallback<QBRTCSession> {
        override fun onDisconnectedFromUser(session: QBRTCSession?, userId: Int?) {
            Log.i(TAG, "CallService --- onDisconnectedFromUser 断开连接 userId = $userId ---")
            WebRtcSessionManager.removeMultiCallUser(userId)
        }

        override fun onConnectedToUser(session: QBRTCSession?, userId: Int?) {
            Log.i(TAG, "CallService --- onConnectedToUser --- userId = " + userId)
            stopRingtone()
            isConnectedCall = true
            startCallTimer()
//            if (CallFloatBoxView.isCallFloatBoxShown() && CallFloatBoxView.isVideoCall()) {
//                Log.i(TAG, "CallService --- onConnectedToUser --- 显示小窗")
//                CallFloatBoxView.hideFloatBox()
//                CallFloatBoxView.showFB(Applications.getCurrent().applicationContext, null, true, videoTrackMap, true, CallFloatBoxView.isIncomingCall())
//            }
        }

        override fun onConnectionClosedForUser(session: QBRTCSession?, userID: Int?) {
            Log.i(TAG, "CallService --- onConnectionClosedForUser 被 $userID 关闭连接,用户离开---")
            WebRtcSessionManager.removeMultiCallUser(userID)
            removeVideoTrack(userID)
        }

        override fun onStateChanged(session: QBRTCSession?, sessionState: BaseSession.QBRTCSessionState?) {
            Log.i(TAG, "CallService --- onStateChanged --- state = $sessionState" + ", session = " + session)
            if (sessionState == QBRTCSessionState.QB_RTC_SESSION_CONNECTED) {
                CallFloatBoxView.updateSessionState(true)
            } else if (sessionState == QBRTCSessionState.QB_RTC_SESSION_CLOSED || sessionState == QBRTCSessionState.QB_RTC_SESSION_GOING_TO_CLOSE) {
                CallFloatBoxView.updateSessionState(false)
            }
        }
    }
...
//            ToastUtils.show(R.string.dlg_signal_error)
        }
    }
...
//            ToastUtils.show("Internet connection " + if (availableNow) "available" else " unavailable")
    ...
    private inner class CameraEventsListener : CameraVideoCapturer.CameraEventsHandler {
        ...
        override fun onCameraFreezed(s: String) {
            Log.i(TAG,"Camera freezed: $s")
            hangUpCurrentSession(HashMap())
        }

        override fun onCameraOpening(s: String) {
            Log.i(TAG,"Camera opening: $s")
        }
    ...
    private inner class VideoTrackListener : QBRTCClientVideoTracksCallbacks<QBRTCSession> {
        override fun onLocalVideoTrackReceive(session: QBRTCSession?, videoTrack: QBRTCVideoTrack?) {
            ...
            Log.d(TAG, "CallService -- onLocalVideoTrackReceive ---")
        }
        ...
        override fun onRemoteVideoTrackReceive(session: QBRTCSession?, videoTrack: QBRTCVideoTrack?, userId: Int?) {
            ...
            Log.d(TAG, "CallService -- onRemoteVideoTrackReceive --- for opponent= $userId")
        }
    }
