package com.legend.baseui.ui.widget.susindexbar.indexbar.helper;

import com.legend.baseui.ui.widget.susindexbar.HanziToPinyin;
    ...
    public IIndexBarDataHelper fillInexTag(List<? extends BaseIndexPinyinBean> datas) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (indexPinyinBean.isNeedToPinyin()) {
                ...
                if (tagString.matches("[A-Z]") || tagString.matches("[a-z]")) {//如果是A-Z字母开头
                    indexPinyinBean.setBaseIndexTag(tagString.toUpperCase());
                } else {//特殊字母这里统一用#处理
