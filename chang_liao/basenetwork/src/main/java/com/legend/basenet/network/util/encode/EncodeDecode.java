...
class EncodeDecode {
     ...
     *
     * @param integer_pixel_array  integer_pixel_array {The integer RGB array}
     * @param image_columns image_columns {Image width}
     * @param image_rows image_rows {Image height}
     * @param messageEncodingStatus messageEncodingStatus {object}
     * @param progressHandler progressHandler {A handler interface, for the progress bar}
     ...
     *
     * @param splitted_images: splitted_images {list of chunk images}
     * @param encrypted_message: encrypted_message {string}
     * @param progressHandler: progressHandler {Progress callback}
    ...
    public static List<Bitmap> encodeMessage(List<Bitmap> splitted_images,
        ...
        encrypted_message = encrypted_message + END_MESSAGE_CONSTANT;
        encrypted_message = START_MESSAGE_CONSTANT + encrypted_message;
     ...
     *
     * @param byte_pixel_array : byte_pixel_array {The byte array image}
     * @param image_columns : image_columns {Image width}
     * @param image_rows : image_rows {Image height}
     * @param messageDecodingStatus : messageDecodingStatus {object}
     */
    private static void decodeMessage(byte[] byte_pixel_array, int image_columns,
        ...
        for (byte aByte_pixel_array : byte_pixel_array) {
            ...
            if (shiftIndex % toShift.length == 0) {
                ...
                if (messageDecodingStatus.getMessage().endsWith(END_MESSAGE_CONSTANT)) {
                    ...
                    if (messageDecodingStatus.getMessage().length() == START_MESSAGE_CONSTANT.length()
                            && !START_MESSAGE_CONSTANT.equals(messageDecodingStatus.getMessage())) {
        ...
        if (!Utility.isStringEmpty(messageDecodingStatus.getMessage()))
            ...
            try {
                messageDecodingStatus.setMessage(
                        messageDecodingStatus.getMessage().substring(START_MESSAGE_CONSTANT.length(),
                                messageDecodingStatus.getMessage().length() - END_MESSAGE_CONSTANT.length()));
            } catch (Exception e) {
                e.printStackTrace();
            }
